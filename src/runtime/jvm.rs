use crate::class_loader::class_file;
use crate::class_loader::constant_pool;
use crate::class_loader::constant_pool::CpInfoInfoEnum;
use crate::class_loader::loader::ClassLoader;
use crate::notimpl;
use crate::runtime::errors;
use crate::runtime::frame;
use crate::runtime::heap;
use crate::runtime::nativememory::NativeMemory;
...
impl JVM {
    ...
    fn bootstrap_user_module(&mut self, user_dir: &str) -> Result<()> {
        ...
        let packages = get_class_names_in_module(user_dir);
    ...
    fn bootstrap_java_base(&mut self) -> Result<()> {
        ...
        let path = format!("{}/{}", get_env("JHOME"), "java.base");
        // MUST load the modules before anything else (used during class resolution !)
        let packages = get_class_names_in_module(&path);
        self.class_loader.modulemanager.add(
            module_java_base_ref,
            true,
            None,
        ...
        let module_descriptor = self.alloc_module_descriptor("java.base", "", "", vec![])?;
    ...
    fn bootstrap(&mut self, user_dir: &str) -> Result<()> {
        ...
        self.bootstrap_user_module(user_dir)?;
    ...
    pub(crate) fn get_current_frame(&self) -> Result<&frame::Frame> {
        self.frames
            .last()
            .ok_or_eyre(errors::RuntimeError::FrameNotFound)
    }
    ...
    fn create_main_thread(&mut self, name: &str) -> Result<types::Type> {
        ...
        self.class_loader
            ...
            .put_static("nextThreadID", types::Type::Long(self.next_thread_id))?;
        self.class_loader
            .get_mut("java/lang/Thread")?
    ...
    pub fn alloc_module_descriptor(
        ...
        let name_obj = self.heap.alloc_string(&mut self.class_loader, name)?;
    ...
    pub fn run(&mut self) -> Result<()> {
        loop {
            match self.step() {
                ...
                Err(err) if err.downcast_ref::<errors::RuntimeError>().is_some() => {
                    let my_error = err.downcast_ref::<errors::RuntimeError>().unwrap();
                    match my_error {
                        errors::RuntimeError::FrameNotFound => {
                            // done here
                            break;
                        }
                        e => {
    ...
    pub(crate) fn pop_string(&mut self) -> Result<String> {
        ...
        Err(eyre!(errors::RuntimeError::GeneralException(
            "failed pop string from stack"
        )))
    }

    pub(crate) fn pop_class(&mut self) -> Result<String> {
        let class_ref = self.pop_object_ref()?;
        let class = self.heap.get_obj_instance(class_ref)?;
        debug!("{:?}", class);
        assert!(class.is_class());

        let raw_value_ref = class.get_field("name")?;
        match raw_value_ref {
            types::Type::ObjectRef(value_ref) => {
                ...
                if let types::Type::ArrayRef(array_ref) = value_field {
                    let array = self.heap.get_array_instance(array_ref)?;
                    let class_name = array.get_string()?;
    ...
    pub(crate) fn pop_array_ref(&mut self) -> Result<ju4> {
        ...
        Err(eyre!(errors::RuntimeError::GeneralException(
            "failed pop array ref from stack"
        )))
    }
    ...
    pub(crate) fn pop_object_ref(&mut self) -> Result<ju4> {
        ...
        Err(eyre!(errors::RuntimeError::GeneralException(
            "failed pop object ref from stack"
        )))
    }
    ...
    pub(crate) fn pop_ref(&mut self) -> Result<ju4> {
        ...
        let obj_ref = match popped {
            ...
            types::Type::Null => 0,
            _ => {
                return Err(eyre!(errors::RuntimeError::GeneralException(
                    "failed pop object ref from stack"
                )))
            }
        };
    ...
    fn exec_baload(&mut self) -> Result<Option<types::Type>> {
        ...
        let array_ref = self.pop_stack()?;
        match array_ref {
            types::Type::ArrayRef(array_ref) if index.is_integer() => {
                let array = self.heap.get_array_instance(array_ref)?;
                let i = index.as_short()?;
                let element = *array.get(i as usize)?;
    ...
    fn exec_aaload(&mut self) -> Result<Option<types::Type>> {
        ...
        match (array_ref, index) {
            (types::Type::ArrayRef(array_ref), types::Type::Integer(i)) => {
                ...
                debug!(
                    "        [~] AALOAD {:?} at {:?} -> {:?}",
                    array_ref, index, array
                );
                let value = array.get(i as usize)?;
                if let types::Type::ObjectRef(obj_ref) = value {
                    let obj = self.heap.get_obj_instance(*obj_ref)?;
                    debug!("        [~] AALOAD {:?}", obj);
                }
                self.push_stack(*array.get(i as usize)?)?;
                Ok(None)
            }
            v => Err(eyre!(format!("ArrayRef not in the stack {:?}", v))),
        }
    ...
    fn exec_aastore(&mut self) -> Result<Option<types::Type>> {
        ...
        let array = match array_ref {
            ...
            _ => return Err(eyre!(errors::RuntimeError::NullPointerException)),
        };
        debug!(
            "        [~] AASTORE {:?} {:?} {:?} to {:?}",
            value,
        ...
        let element_class_name = array.get_element_classname();
        let value = match value {
            ...
            _ => {
                return Err(eyre!(errors::RuntimeError::WrongType(
                    "ArrayRef/Null",
                    value
                )))
            }
    ...
    fn exec_ldc(&mut self) -> Result<Option<types::Type>> {
        let value: types::Type = {
            ...
            match info {
                ...
                constant_pool::CpInfoInfoEnum::Long(_)
                | constant_pool::CpInfoInfoEnum::Double(_) => {
    ...
    fn exec_invokestatic(&mut self) -> Result<Option<types::Type>> {
        ...
        let max_locals = method
            .get_code()
            .ok_or_eyre(eyre!(errors::RuntimeError::GeneralException(
                "code not found"
            )))?
            .get_max_locals();
                ...
                types::Type::ObjectRef(obj_ref) => {
                    ...
                    if obj.get_classname() == "java/lang/String" {
                        ...
                        if let types::Type::ArrayRef(array_ref) = array_field {
                            ...
                            debug!("Argument {:?} {:?} {:?}", obj_ref, array_ref, name);
                        }
                    } else {
                        debug!("Argument {:?}", obj)
                    }
            ...
            match self.pop_stack()? {
                a @ (types::Type::ObjectRef(obj_ref) | types::Type::ArrayRef(obj_ref)) => {
                    ...
                    if !types::Type::check_type(
                        ...
                        panic!("WRONG TYPE {} {}", class_name, obj.get_class_field_type());
                    }
                    args.push(a);
                }
                types::Type::Null => return Err(eyre!(errors::RuntimeError::NullPointerException)),
                v => return Err(eyre!(errors::RuntimeError::WrongType("ObjectRef", v))),
            };
        }
    ...
    fn exec_invokespecial(&mut self) -> Result<Option<types::Type>> {
        ...
        let max_locals = self.class_loader.get_method_max_locals_by_name(
            &class_name,
            &method_name,
            &method_type,
        )?;
    ...
    fn exec_invokeinterface(&mut self) -> Result<Option<types::Type>> {
        ...
        // TODO:  if the resolved method is static, the invokeinterface instruction throws an IncompatibleClassChangeError.
    ...
    fn exec_invokedynamic(&mut self) -> Result<Option<types::Type>> {
        ...
        let curr_class_name = curr_class.get_name().to_owned();
        let _data = curr_class.resolve_index(index)?;
        //debug!("{:?}", data);
        drop(curr_class);
        let (_class_name, _method_name, _method_type) = self
            .class_loader
            .resolve_invokedynamic(&curr_class_name, index)?;

        //Ok(None)
        Err(eyre!(errors::RuntimeError::NotImplementedException))
    }
    ...
    fn exec_putfield(&mut self) -> Result<Option<types::Type>> {
        ...
        debug!(
            "        [~] Put field {:?} {:?} in {:?} -> {:?}",
            field_name, field_type, class_name, value
        );
        // objrect.field = value
        match objref {
            types::Type::ObjectRef(reference) => {
                ...
                debug!(
                    "field type: {:?}",
                    types::Type::field_type_to_type(&field_type)
                );
                debug!(
                    "value type: {:?}",
                    types::Type::field_type_to_type(&value_type)
                );
                if let types::Type::ObjectRef(value_ref) = value {
                    let value = self.heap.get_obj_instance(value_ref)?;
                    debug!(
                        "value type: {:?} {:?}",
                        types::Type::field_type_to_type(&value_type),
                        value
    ...
    fn exec_checkcast(&mut self) -> Result<Option<types::Type>> {
        ...
        match info {
            CpInfoInfoEnum::Class(class_info) => {
                ...
                //  If objectref is a value of the type given by the resolved class, array, or interface type, the operand stack is unchanged.
    ...
    fn exec_athrow(&mut self) -> Result<Option<types::Type>> {
        ...
        let _obj_ref = self.pop_stack()?;

        // TODO: check type is Throwable or subclass
        // TODO: https://docs.oracle.com/javase/specs/jvms/se23/html/jvms-2.html#jvms-2.10
        Err(eyre!(errors::RuntimeError::NotImplementedException))
    }
    ...
    fn exec_tableswitch(&mut self) -> Result<Option<types::Type>> {
        ...
        if low >= high {
            return Err(eyre!(errors::RuntimeError::GeneralException(
                "low should be less than high in tableswitch"
            )));
        }
        debug!(
    ...
    fn exec_lookupswitch(&mut self) -> Result<Option<types::Type>> {
        ...
        debug!("{:?}", self.get_current_frame()?.get_code());
        let default = self.get_current_frame_mut()?.read_ju4()? as i32;
    ...
    fn exec_arraylength(&mut self) -> Result<Option<types::Type>> {
        ...
        match value {
            ...
            _ => return Err(eyre!(format!("ArrayRef not in the stack {:?}", value))),
        }
        Ok(None)
    }
    ...
    pub(crate) fn get_current_class(&mut self) -> Result<RwLockReadGuard<class_file::ClassFile>> {
        let current_class = self.get_current_frame()?.get_classname().clone();
    ...
    pub(crate) fn call_and_resolve_method(
        ...
        info!("[-] Resolving {:?} {:?}", class_name, method_name);
        ...
        while !self.frames.is_empty()
            ...
            && *self.frames.get(num_frames - 1).unwrap() == cpframe
        {
            match self.step() {
                Ok(v) => {
                    if let Some(v) = v {
        ...
        info!(
            "[!] Resolved {:?} {:?} {:?}",
            class_name, method_name, returned
        );
        Ok(returned)
    }
    ...
    pub(crate) fn step(&mut self) -> Result<Option<types::Type>> {
        ...
        debug!(
            ...
            class_name,
            current_frame.get_func_name(),
            current_frame.get_func_type(),
        );
        ...
        if !self.frames.is_empty()
            && (self.get_current_frame()?.get_func_name() == "writeln"
                || self.get_current_frame()?.get_func_name() == "println")
        {
            //pause();
        }
