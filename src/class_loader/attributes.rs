...
impl Code {
    fn parse(bytes: &[u8], cp: &ConstantPool) -> Result<(Code, usize)> {
        ...
        for _ in 0..(exception_table_length as usize) {
            ...
            let (excep, bytes_read) = ExceptionTableEntry::parse(slice)?;
            index += bytes_read;
    ...
    pub fn get_max_locals(&self) -> ju2 {
        self.max_locals
    }
    pub fn get_code_vec(&self) -> &Vec<ju1> {
        &self.code
    }
}
...
pub struct InnerClassesClass {
    ...
    inner_name_index: ju2,
    inner_class_access_flags: ju2,
}
impl InnerClassesClass {
    fn parse(bytes: &[u8]) -> Result<(InnerClassesClass, usize)> {
...
impl InnerClasses {
    fn parse(bytes: &[u8]) -> Result<(InnerClasses, usize)> {
        let number_of_classes = ju2_from_bytes(&bytes[0..2])?;
        let mut classes: Vec<InnerClassesClass> = vec![];
        let mut index = 2;
        for _ in 0..number_of_classes {
            let slice = &bytes[index..];
            let (class, bytes_read) = InnerClassesClass::parse(slice)?;
            classes.push(class);
            index += bytes_read;
        }
        Ok((
...
impl ElementValue {
    fn parse(bytes: &[u8]) -> Result<(ElementValue, usize)> {
        let element_name_index = ju2_from_bytes(&bytes[0..2])?;
        let tag = bytes[2];
        let (value, bytes_read) = match tag {
            b'B' | b'C' | b'D' | b'F' | b'I' | b'J' | b'S' | b'Z' | b's' => {
                let const_value_index = ju2_from_bytes(&bytes[3..5])?;
                (ElementValueEnum::ConstValue { const_value_index }, 3 + 2)
            }
            b'e' => {
                let type_name_index = ju2_from_bytes(&bytes[3..5])?;
                let const_name_index = ju2_from_bytes(&bytes[5..7])?;
                (
            ...
            b'c' => {
                let class_info_index = ju2_from_bytes(&bytes[3..5])?;
                (ElementValueEnum::ClassInfo { class_info_index }, 3 + 2)
            }
            b'@' => {
                let (annotation, bytes_read) = Annotation::parse(&bytes[3..])?;
                (
            ...
            b'[' => {
                let num_values = ju2_from_bytes(&bytes[3..5])?;
                let mut values: Vec<ElementValue> = vec![];
                let mut index = 5;
                for _ in 0..num_values {
                    let (element, bytes_read) = ElementValue::parse(&bytes[index..])?;
                    index += bytes_read;
                    values.push(element);
                }
                (ElementValueEnum::ArrayValue(values), index)
            }
            _ => return Err(eyre!("tag not supported in element value: {:?}", tag)),
        };
...
pub struct Annotation {
    ...
    element_value_pairs: Vec<ElementValue>,
}
impl Annotation {
    fn parse(bytes: &[u8]) -> Result<(Annotation, usize)> {
        let type_index = ju2_from_bytes(&bytes[0..2])?;
        let num_element_value_pairs = ju2_from_bytes(&bytes[2..4])?;
        let mut element_value_pairs: Vec<ElementValue> = vec![];
        let mut index = 4;
...
impl RuntimeVisibleAnnotations {
    fn parse(bytes: &[u8]) -> Result<(RuntimeVisibleAnnotations, usize)> {
        let num_annotations = ju2_from_bytes(&bytes[0..2])?;
        let mut annotations: Vec<Annotation> = vec![];
        let mut index = 2;
        for _ in 0..num_annotations {
            let (annotation, bytes_read) = Annotation::parse(&bytes[index..])?;
            index += bytes_read;
...
impl RuntimeInvisibleAnnotations {
    fn parse(bytes: &[u8]) -> Result<(RuntimeInvisibleAnnotations, usize)> {
        let num_annotations = ju2_from_bytes(&bytes[0..2])?;
        let mut annotations: Vec<Annotation> = vec![];
        let mut index = 2;
        for _ in 0..num_annotations {
...
impl EnclosingMethod {
    fn parse(bytes: &[u8]) -> Result<(EnclosingMethod, usize)> {
...
impl AttributeInfo {
    pub fn parse(bytes: &[u8], cp: &ConstantPool) -> Result<(AttributeInfo, usize)> {
        ...
        let (info, bytes_read) = match attribute_name.as_ref() {
            ...
            "InnerClasses" => {
                let (inner_classes, bytes_read) = InnerClasses::parse(info_slice)?;
                (
            ...
            "RuntimeVisibleAnnotations" => {
                let (annotations, bytes_read) = RuntimeVisibleAnnotations::parse(info_slice)?;
                (
            ...
            "RuntimeInvisibleAnnotations" => {
                let (annotations, bytes_read) = RuntimeInvisibleAnnotations::parse(info_slice)?;
                (
            ...
            "EnclosingMethod" => {
                let (method, bytes_read) = EnclosingMethod::parse(info_slice)?;
...
pub struct Attributes {
    attributes_count: ju2,
    attributes: Vec<AttributeInfo>,
}
impl Attributes {
    pub fn new(attributes_count: ju2, attributes: Vec<AttributeInfo>) -> Attributes {
        Attributes {
            attributes_count,
            attributes,
        }
