...
impl RefInfo {
    ...
    #[allow(dead_code)]
    pub(crate) fn get_name_and_type_index(&self) -> ju2 {
...
impl CpInfoInfoEnum {
    fn from_tag(tag: &CpInfoTagEnum, bytes: &[u8]) -> Result<CpInfoInfoEnum> {
        let res = match tag {
            ...
            CpInfoTagEnum::NameAndType => {
                CpInfoInfoEnum::NameAndType(NameAndTypeInfo::parse(bytes)?)
            }
            ...
            CpInfoTagEnum::MethodHandle => {
                CpInfoInfoEnum::MethodHandle(MethodHandleInfo::parse(bytes)?)
            }
            _ => {
...
impl ConstantPool {
    ...
    pub(crate) fn _resolve_ref(&self, index: ju2) -> Result<(ju2, ju2, ju2)> {
        let value: &CpInfo = self.resolve_index(index)?;
        let refinfo = match &value.info {
            CpInfoInfoEnum::Fieldref(refinfo) => refinfo,
            CpInfoInfoEnum::Methodref(refinfo) => refinfo,
            CpInfoInfoEnum::Interfaceref(refinfo) => refinfo,
            _ => return Err(eyre!("[!] Index {:?} is not Ref but {:?}", index, value)),
        };
        let class_item = self.resolve_index(refinfo.get_class_index())?;
        let class = match &class_item.info {
            CpInfoInfoEnum::Class(info) => info.name_index,
            _ => {
                return Err(eyre!(
                    "[!] Index {:?} is not Class but {:?}",
                    index,
        ...
        let name_and_type = self.resolve_index(refinfo.get_name_and_type_index())?;
        let (name, _type) = match &name_and_type.info {
            CpInfoInfoEnum::NameAndType(info) => (info.name_index, info.descriptor_index),
            _ => {
                return Err(eyre!(
                    "[!] Index {:?} is not Class but {:?}",
                    index,
