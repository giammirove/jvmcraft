...
pub fn generate_hash(classname: &str, obj_ref: ju4) -> ju4 {
  let nonce = generate_nonce();

  // Hash the reference to the object (memory address)
  let str_to_hash = format!("{}-{}-{}", classname, obj_ref, nonce);

  let mut hasher = DefaultHasher::new();

  str_to_hash.hash(&mut hasher);

  hasher.finish() as ju4
}
...
pub(crate) fn get_return_type_descriptor(desc: &str) -> String {
  descriptor_to_classname(desc.split(')').nth(1).unwrap())
}

pub(crate) fn get_parameters_type_descriptor(desc: &str) -> &str {
  let open = desc.find('(').unwrap();

  let close = desc.find(')').unwrap();

  &desc[open + 1..close]
}

pub(crate) fn parse_parameter_types(params: &str) -> Vec<&str> {
  let mut types = vec![];
...
pub(crate) fn descriptor_to_classname(desc: &str) -> String {
  match desc {
...
pub(crate) fn get_array_element_class_name(desc: &str) -> String {
  match desc {
    d if d.starts_with('[') => d[1..d.len()].to_string(),
    _ => desc.to_string(),
  }
}

pub(crate) fn classname_to_descriptor(classname: &str) -> String {
  if classname.contains("/") {
    return format!("L{};", classname);
  }
  classname.to_owned()
}

pub(crate) fn get_argument_classnames(descriptor: &str) -> Vec<String> {
  let param_str = get_parameters_type_descriptor(descriptor);

  let types = parse_parameter_types(param_str);

  types.into_iter().map(descriptor_to_classname).collect()
}
...
pub(crate) fn get_classnames_in_module(root: &str) -> Vec<String> {
  let mut files = vec![];
...
#[inline]
pub(crate) fn class_to_dotclass(classname: &str) -> String {
  classname.replace("/", ".")
}

#[inline]
pub(crate) fn dotclass_to_class(classname: &str) -> String {
  classname.replace(".", "/")
}
